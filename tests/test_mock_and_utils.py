...
def test_utils_match_string_via_api_returns_string(monkeypatch):
    ...
    def _fake_http_get(url: str, timeout_s: float = 60.0):
        ...
        assert (
            "candidates=Cabinet+Office" in url or "candidates=Cabinet%20Office" in url
        )
        assert (
            "prompt_path=prompts%2Fbuyer_match_v1.txt" in url
            or "prompt_path=prompts/buyer_match_v1.txt" in url
        )
        return (
            200,
            '{"input_string":"Home Office","match":"Cabinet Office","raw":"Cabinet Office"}',
        )

    monkeypatch.setenv("MATCH_STRING_API_URL", "http://example.test/match")
    monkeypatch.setattr("utils._http_get", _fake_http_get)

    out = match_string_via_api(
        input_string="Home Office", list_of_strings=cands, prompt_path=prompt_path
    )
