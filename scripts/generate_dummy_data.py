...
def generate_dummy_mi_data():
    ...
    Row 14: an MI entry using incorrect buyer name "Buyer A Department" - first instance (edge case: should match to "Buyer A")
    Row 15: an MI entry using the same incorrect buyer name "Buyer A Department" - second instance by different supplier (edge case: both uses same wrong name)
    Row 16: an MI entry using incorrect buyer name "DWP Office" - first instance (edge case: should match to "Department for Work and Pensions")
    Row 17: an MI entry using the same incorrect buyer name "DWP Office" - second instance by different supplier (edge case: both uses same wrong name)
    """
    data = {
