    ...
    for framework, group in df.groupby("framework"):
        ...
        for supplier in suppliers_list:
            contract_start_date = datetime.strptime(
                supplier["contract_start"], "%Y-%m-%d"
            )
            contract_end_date = datetime.strptime(supplier["contract_end"], "%Y-%m-%d")
            today = datetime.now()
            if contract_end_date < today:
                months_ran = (
                    contract_end_date.year - contract_start_date.year
                ) * 12 + (contract_end_date.month - contract_start_date.month)
            else:
                months_ran = (today.year - contract_start_date.year) * 12 + (
                    today.month - contract_start_date.month
                )
            supplier["details"] = {
...
def index():
    ...
    return render_template(
        "index.html", frameworks=frameworks, suppliers=initial_suppliers
    )
